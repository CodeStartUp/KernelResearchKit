...
void DisplayBanner() {
    ...
    std::wcout << L"+----------------------------------------------------------+\n";
    std::wcout << L"|                                                             |\n";
    std::wcout << L"|               Windows driver loader                         |\n";
    std::wcout << L"|                                                             |\n";
    std::wcout << L"|                                                             |\n";
    std::wcout << L"+----------------------------------------------------------+\n";
    std::wcout << L"\n";
...
// ============================================================================
// ============================================================================
// AUTO-LOAD DRIVER FROM EXE DIRECTORY
// ============================================================================

bool AutoLoadDriver(DrvLoader& loader, const std::wstring& driverName) {
    ...
    std::wstring driverPath = exeDir + driverName;

    std::wcout << L"\n=========================================================\n";
    std::wcout << L"              AUTO-LOADING DRIVER\n";
    std::wcout << L"=========================================================\n";
    std::wcout << L"[*] Looking for driver: " << driverPath << L"\n";

    // Check if driver exists
    if (GetFileAttributesW(driverPath.c_str()) == INVALID_FILE_ATTRIBUTES) {
        std::wcout << L"[-] Driver not found: " << driverPath << L"\n";
        std::wcout << L"[*] Make sure " << driverName << L" is in the same folder as this executable\n";
        return false;
    }

    std::wcout << L"[+] Driver found: " << driverPath << L"\n";

    // Load the driver
    bool success = loader.LoadDriver(driverPath, SERVICE_DEMAND_START);

    if (success) {
        std::wcout << L"\n[SUCCESS] Driver auto-loaded successfully!\n";
    }
    else {
        std::wcout << L"\n[FAILED] Driver auto-load failed!\n";
    }
